...
```
# Fit Mandarin and English data using lmer
```{r}
library(lmerTest)
lmeM = lmer(VOT~gender + (1 | subject) + (gender | item), data = datM_stops)
summary(lmeM)

lmeE = lmer(VOT~gender + (1 | subject) + (gender | item), data = datE_stops)
summary(lmeE)
```
...
m1M <- brm(formula = VOT ~ gender + (1 | subject) + (gender | item),
           ...
           iter = 2000, chains = 4, control = list(adapt_delta = 0.99))
```
## summarize default priors model
...
priors_N20  <- c(set_prior("normal(30, 50)", class = "Intercept"),
              set_prior("normal(0,20)", class = "b", coef="gender"),
              set_prior("normal(0, 100)", class = "sd"),
              set_prior("normal(0, 100)", class = "sigma"),
              set_prior("lkj(2)", class = "cor"))
...
priors_N20_0  <- c(set_prior("normal(30, 50)", class = "Intercept"),
              #set_prior("normal(0,20)", class = "b", coef="gender"),
              set_prior("normal(0, 100)", class = "sd"),
              set_prior("normal(0, 100)", class = "sigma"),
              set_prior("lkj(2)", class = "cor"))
...
loo(m1M, m0M)
```
# Fit locus equation data
lmer and Bayesian mixed effects models

## unnormalized data default priors
Does not converge in 2000 iterations with default priors

```{r}
CVC = read_csv("../2 Linear Models/engCVC.csv")
...
bCVC <- brm(formula = F2C1 ~ C1*F2V + (C1 + F2V| speaker),
           data = CVC, family = gaussian(), prior = set_prior("normal(2305, 300)", class = "Intercept"),
           iter = 2000, chains = 4, control = list(adapt_delta = 0.99))
```
...
```{r}
plot(bCVC, variable = c("Intercept", "b_F2V"))
```


# non-default priors
does converge as long as model is specified 0 + Intercept
priors on random effect sd's need to specified separately for C and F2V given their different scales.
(Previous version had all these priors as normal(0,1)!)

```{r}

priors <- c(set_prior("normal(500, 300)", class = "b", coef = "Intercept"),
                      ...
                      set_prior("normal(0, 1)", class = "b",
                                coef = "C1g:F2V"),
                      set_prior("normal(0, 100)", class = "sd"),
                      set_prior("normal(0,0.5)", class = "sd", group = "speaker", coef = "F2V"),
                      set_prior("normal(0, 200)", class = "sigma"),
                      set_prior("lkj(2)", class = "cor"))

bCVC_int <- brm(formula = F2C1 ~ 0 + Intercept + C1*F2V + (C1 + F2V| speaker),
           data = CVC, family = gaussian(), prior = priors,
           iter = 2000, chains = 4, control = list(adapt_delta = 0.99))
```
...
## plot chains
```{r}
plot(bCVC_int, variable = c("b_Intercept", "b_F2V"))
```

### try to translate successful prior to centered intercept 
500 + 0.7 * *mean(CVC$F2V, na.rm = TRUE) = 1821
reduced sd's of priors on C1
other priors as above
Given these priors, it is not necessary to reduce the sd on the prior on the intercept

```{r}

priors <- c(set_prior("normal(1821, 300)", class = "Intercept"),
                      set_prior("normal(0, 1)", class = "b", 
                                coef = "F2V"),
                      set_prior("normal(0, 500)", class = "b",
                                coef = "C1d"),
...
bCVC_int_center <- brm(formula = F2C1 ~ 1 + C1*F2V + (C1 + F2V| speaker),
           data = CVC, family = gaussian(), prior = priors,
           iter = 2000, chains = 4, control = list(adapt_delta = 0.99))
```
...
```{r}
summary(bCVC_int_center)
```
```{r}
plot(bCVC_int_center, variable = c("Intercept", "b_F2V", "b_C1d"))
```

## scaled predictors
informative priors

```{r fitCVCdata,cache=TRUE,warning=FALSE,message=FALSE}

lmec4 = lmer(scale(F2C1)~C1*scale(F2V)+(C1|speaker), data = CVC)
summary(lmec4)

lmec5 = lmer(scale(F2C1)~C1*scale(F2V)+(scale(F2V)|speaker), data = CVC)
summary(lmec5)
...
CVC$F2V_scale = scale(CVC$F2V)

bCVC_scale <- brm(formula = scale(F2C1) ~ C1*F2V_scale + (C1 + F2V_scale| speaker),
           data = CVC, family = gaussian(), prior = priors,
...
summary(bCVC_scale)
```
# try maximal random effects

```{r}
bCVC_maximal <- brm(formula = scale(F2C1) ~ C1*F2V_scale + (C1 * F2V_scale| speaker),
           data = CVC, family = gaussian(), prior = priors,
           iter = 2000, chains = 4, control = list(adapt_delta = 0.99))
```
Summarize the model fit:
...
```{r summaryCVCdata}
summary(bCVC_maximal)
```

```{r stanplotCVCdata,warning=FALSE,message=FALSE}
stanplot(bCVC_maximal, pars = c("^b","sd","sigma"), type="hist")
```
## Posterior predictive check

```{r posteriorpredictivecheck}
pp_check(bCVC_maximal, ndraws = 100)+
  theme(text = element_text(size=16),legend.text=element_text(size=16))
```

## maximal random effects, default priors

```{r}
bCVC_maximal_default <- brm(formula = scale(F2C1) ~ C1*F2V_scale + (C1 * F2V_scale| speaker),
           data = CVC, family = gaussian(), #prior = priors,
           iter = 2000, chains = 4, control = list(adapt_delta = 0.99))
```
Summarize the model fit:
...
```{r summaryCVCdata}
summary(bCVC_maximal_default)
```

## plot posterior

```{r}
stanplot(bCVC_maximal_default, pars = c("^b","sd","sigma"), type="hist")
```

# default priors except for correlations among random effects

```{r}
priors <- c(set_prior("lkj(2)", class = "cor"))

bCVC_maximal_LKJ2 <- brm(formula = scale(F2C1) ~ C1*F2V_scale + (C1 * F2V_scale| speaker),
           data = CVC, family = gaussian(), prior = priors,
           iter = 2000, chains = 4, control = list(adapt_delta = 0.99))
```
...
```{r}
summary(bCVC_maximal_LKJ2)
```

## plot posterior

```{r}
stanplot(bCVC_maximal_LKJ2, pars = c("^b","sd","sigma"), type="hist")
```

# Analyses of affricate data

```{r}
# load affricate data
affricate_data <- read_csv("../2\ Linear\ Models/affricate_data.csv")
...
# scale intensity
aff$intensity_scale = scale(aff$intensity)
```

Fit brm with default priors

```{r}
aff.brm = brm(formula = intensity_scale ~ affricate*context + (1 + affricate*context| subject) + (1+context|word),
           data = aff, family = gaussian(),
           iter = 2000, chains = 4, control = list(adapt_delta = 0.99))
```
...
```{r}
# simplify
lme_aff_red1 = lmer(scale(intensity)~affricate*context+(1+affricate+context|subject)+(context|word), data=aff)
summary(lme_aff_red1)
```
plot posterior

```{r}
stanplot(aff.brm, pars = c("^b","sd","sigma"), type="hist")
```
...
aff.brm.LKJ.normsd = brm(formula = intensity_scale ~ affricate*context + (1 + affricate*context| subject) + (1+context|word), 
      prior = priors,
      data = aff, family = gaussian(),
      iter = 2000, chains = 4, control = list(adapt_delta = 0.99))
```
```{r}
summary(aff.brm.LKJ.normsd)
```
